...
func listTrips(_ trips: [Trip], _ reference: [String:String]?, _ workflow: Workflow) {
  ...
  trips.forEach { (trip) in
    ...
    let item = Item(title: title, subtitle: tripSubtitle(subtitle), icon: Item.Icon(path: "./icons/trip\(expired ? "_exp" : "").png"), text: Item.Text(copy: timeTable(trip)), variables: ["tripId": trip.id])
    workflow.add(item)
  }
